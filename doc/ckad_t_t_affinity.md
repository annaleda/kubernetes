...
| Categoria           | Operator         | Dove si usa      | Quando usarlo                     | Caso tipico CKAD                      | Note importanti             |
| ------------------- | ---------------- | ---------------- | --------------------------------- | ------------------------------------- | --------------------------- |
| **NodeSelector**    | Equal (implicit) | nodeSelector     | Match esatto label=value          | disktype=ssd                          | Forma più semplice          |
| **NodeAffinity**    | In               | matchExpressions | Lista valori ammessi              | color in [blue, green]                |  Più usato                  |
| **NodeAffinity**    | NotIn            | matchExpressions | Escludere valori                  | evitare role=master                   | Scheduling negativo         |
| **NodeAffinity**    | Exists           | matchExpressions | Verificare solo presenza key      | node-role.kubernetes.io/control-plane |  Frequentissimo             |
| **NodeAffinity**    | DoesNotExist     | matchExpressions | Evitare nodi con una label        | evitare maintenance=true              | Molto usato per worker-only |
| **NodeAffinity**    | Gt               | matchExpressions | Confronto numerico >              | cpu > 4                               | Raro nel CKAD               |
| **NodeAffinity**    | Lt               | matchExpressions | Confronto numerico <              | memory < 8                            | Raro                        |
| **PodAffinity**     | In / Exists      | labelSelector    | Stare vicino a pod specifici      | app=frontend                          | Richiede topologyKey        |
| **PodAntiAffinity** | In / Exists      | labelSelector    | Evitare pod specifici             | separare repliche                     | Alta disponibilità          |
| **Tolerations**     | Equal            | tolerations      | Tollerare taint key=value         | dedicated=blue                        | Necessario con NoSchedule   |
| **Tolerations**     | Exists           | tolerations      | Tollerare qualsiasi value per key | key: dedicated                        | Meno comune                 |

---

| Categoria           | Tipo                                            | Effetto      | Quando usarlo            |
| ------------------- | ----------------------------------------------- | ------------ | ------------------------ |
| **NodeAffinity**    | requiredDuringSchedulingIgnoredDuringExecution  | Obbligatorio | “Place only on”          |
| **NodeAffinity**    | preferredDuringSchedulingIgnoredDuringExecution | Preferenza   | “Prefer nodes with”      |
| **PodAffinity**     | requiredDuringSchedulingIgnoredDuringExecution  | Obbligatorio | “Must run with”          |
| **PodAffinity**     | preferredDuringSchedulingIgnoredDuringExecution | Preferenza   | “Prefer to run with”     |
| **PodAntiAffinity** | requiredDuringSchedulingIgnoredDuringExecution  | Obbligatorio | “Must not run with”      |
| **PodAntiAffinity** | preferredDuringSchedulingIgnoredDuringExecution | Preferenza   | “Prefer not to run with” |

---

| Scenario                          | Categoria                   | Operator corretto |
| --------------------------------- | --------------------------- | ----------------- |
| Label key presente senza value    | NodeAffinity                | Exists            |
| Place pod su controlplane         | NodeAffinity                | Exists            |
| Place pod su nodo con label=value | NodeSelector o NodeAffinity | Equal / In        |
| Lista nodi ammessi                | NodeAffinity                | In                |
| Escludere nodi con certa label    | NodeAffinity                | DoesNotExist      |
| Evitare controlplane              | NodeAffinity                | DoesNotExist      |
| Prefer scheduling                 | Affinity                    | preferred         |
| Scheduling obbligatorio           | Affinity                    | required          |
| Taint presente con NoSchedule     | Tolerations                 | Equal             |
| Taint senza value specificato     | Tolerations                 | Exists            |
